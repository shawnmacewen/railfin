...
import { internalCampaignEnrollmentsCreate, internalCampaignEnrollmentsList } from "../../../../../../api/internal/campaigns/contracts";
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ campaignId: string }> }) {
  ...
  const result = await internalCampaignEnrollmentsList({ campaignId });
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ campaignId: string }> }) {
  ...
  const result = await internalCampaignEnrollmentsCreate({ campaignId, body: body ?? undefined });
  const status = !result.ok ? (result.error === "Validation failed" ? 400 : result.error === "Not found" ? 404 : 500) : 201;
  return NextResponse.json(result, { status, headers: INTERNAL_SENSITIVE_NO_STORE_HEADERS });
}
