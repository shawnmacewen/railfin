...
import { internalCampaignEnrollmentTransition } from "../../../../../../../api/internal/campaigns/contracts";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ enrollmentId: string }> }) {
  ...
  const result = await internalCampaignEnrollmentTransition({ enrollmentId, body: body ?? undefined });
