...
async function callChatCompletions(options: {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Codex provider request failed (${response.status}): ${body.slice(0, 300)}`);
    }
    ...
    if (typeof content !== 'string' || !content.trim()) {
      throw new Error('Codex provider returned an empty completion.');
    }

    return content;
  } catch (error) {
    if (error instanceof Error && error.name === 'AbortError') {
      throw new Error(`Codex provider timed out after ${options.timeoutMs}ms.`);
    }
...
export class CodexProvider implements AIProvider {
  async complete(prompt: string): Promise<string> {
    ...
    if (!apiKey) {
      throw new Error('Codex provider is missing API key (CODEX_API_KEY or OPENAI_API_KEY).');
    }
